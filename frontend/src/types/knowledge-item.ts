...
export const ITEM_STATUS_MAP: Record<KnowledgeItemStatus, ItemStatusMeta> = {
  pending: { label: '待处理', color: 'orangered', description: '已创建，等待解析或入库' },
  processing: { label: '处理中', color: 'arcoblue', description: '正在解析、切片或向量化' },
  ready: { label: '可用', color: 'green', description: '可被 RAG 检索引用' },
  failed: { label: '失败', color: 'red', description: '处理失败，可查看错误并重试' },
  disabled: { label: '已禁用', color: 'gray', description: '不参与检索' },
}
